...
def save_model(valid_metric, best_valid_metric, model_dict, model_name, save_dir, metric_name, epoch, fold, mode= 'max'):
    ...
    if mode == 'max':
        if valid_metric>best_valid_metric:
            ...
            save_file_path = os.path.join(save_dir, f'{model_name}_{metric_name}_fold_{fold}.pth')
            torch.save(model_dict, save_file_path)
        else:
            save_file_path = os.path.join(save_dir, f'{model_name}_{epoch}_fold_{fold}.pth')
            torch.save(model_dict, save_file_path)
